...
/*	ProcessManagementPtr->Shutdown.Flags.OpenGL = 0b0;
	...
	Sleep(10);// Wait for 10ms to allow SM to fully start up
	...
	while (1)
	{
		...
		//	ProcessManagementPtr->Shutdown.Flags.Camera = 0b0;
		//}
		//else if (ProcessManagementPtr->Heartbeat.Flags.Camera == 0b1) // Heartbeat still
		//{
		//	cout << "Camera is normal: 1, count=" << cnt_Camera << endl;
		//	ProcessManagementPtr->Heartbeat.Flags.Camera = 0b0; // Clear for next heartbeat checking
		//	cnt_Camera = 0;
		//}
		//else if (ProcessManagementPtr->Heartbeat.Flags.Camera == 0b0) // Heartbeat stop
		//{
		//	cout << "Camera no response: 0, count=" << cnt_Camera << endl;
		//	cnt_Camera++;
		...
		else if (ProcessManagementPtr->Heartbeat.Flags.GPS == 0b1) // Heartbeat still
		{
			cout << "GPS is normal: 1, count=" << cnt_GPS << endl;
			ProcessManagementPtr->Heartbeat.Flags.GPS = 0b0; // Clear for next heartbeat checking
			cnt_GPS = 0;
		}
		else if (ProcessManagementPtr->Heartbeat.Flags.GPS == 0b0) // Heartbeat stop
		{
			cout << "GPS no response: 0, count=" << cnt_GPS << endl;
			cnt_GPS++;
